...
const TH =
  "px-5 py-4 text-[10px] font-black uppercase tracking-widest text-slate-400 whitespace-nowrap";
...
export function AdminUsers() {
  ...
  const isJobsTab =
    location.pathname.includes("vagas") && !location.pathname.includes("nova");
  ...
  const handleRemove = async (id: string, type: "user" | "app" | "job") => {
    if (
      !window.confirm(
        "Confirmar exclusão permanente? Esta ação não pode ser desfeita.",
      )
    )
      return;
    try {
      ...
      const res = await fetch(
        `${import.meta.env.VITE_API_URL}/${map[type]}/${id}`,
        {
          method: "DELETE",
              ...
              {!loading &&
                ...
                jobs.map((job) => (
                  <tr key={job.id} className={ROW}>
                    ...
                    <td className={TD}>
                      <span
                        className={`px-3 py-1 rounded-full text-[10px] font-black border uppercase tracking-wider ${statusColor(job.status)}`}
                      >
                        {job.status}
                      </span>
                    </td>
                    <td
                      className={`${TD} text-center text-[11px] font-bold text-slate-500 uppercase`}
                    >
              ...
              {!loading &&
                ...
                applications.map((app) => (
                  <tr key={app.id} className={ROW}>
                    ...
                    <td className={TD}>
                      <span
                        className={`px-3 py-1 rounded-full text-[10px] font-black border uppercase tracking-wider ${statusColor(app.currentStage)}`}
                      >
                        {app.currentStage}
                      </span>
                    </td>
                    ...
                    <td className={`${TD} text-center`}>
                      <button
                        onClick={() => handleRemove(app.id, "app")}
                        className="text-slate-300 hover:text-red-500 transition-colors p-2 rounded-lg hover:bg-red-50"
                      >
                        <Trash2 size={16} />
                      </button>
