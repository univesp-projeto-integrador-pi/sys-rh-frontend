...
export function CreateJob() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await fetch(
        `${import.meta.env.VITE_API_URL}/jobs-services`,
        {
          method: "POST",
          headers: {
      ...
      if (!response.ok) {
        ...
        if (contentType && contentType.indexOf("application/json") !== -1) {
          ...
          throw new Error(
            `Erro no servidor: Rota não encontrada (Cannot POST) ou inválida.`,
          );
        }
      }
  ...
  const inputStyle =
    "w-full p-4 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-teal-500 outline-none transition-all text-slate-700 disabled:opacity-50";
  const labelStyle =
    ...
    <div className="min-h-screen bg-slate-100 py-12 px-6">
      ...
      >
        <Link
          ...
          className="inline-flex items-center gap-2 text-slate-500 hover:text-teal-600 mb-6 transition-colors font-bold uppercase text-xs tracking-widest"
        >
          ...
          <div className="bg-slate-900 p-8 text-white flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-slate-400 text-xs font-medium">
                Preencha os dados para publicação imediata
              </p>
            </div>
          </div>
          ...
          <form onSubmit={handleSubmit} className="p-8 space-y-6">
            ...
            <div>
              ...
              <textarea
                name="description"
                rows={6}
                placeholder="Descreva as responsabilidades, requisitos técnicos e benefícios..."
                required
