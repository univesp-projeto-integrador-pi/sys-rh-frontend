...
export function Register() {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(
        `https://sys-rh-backend.onrender.com/api/v1/auth/register`,
        {
          method: "POST",
          headers: {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          (body && body.message) || "Falha ao registrar usuário.",
        );
      }
  ...
  return (
    <div className="min-h-screen w-full bg-slate-50 flex items-center justify-center p-6 relative overflow-hidden">
      ...
      <div
        className={`bg-white p-12 rounded-3xl shadow-xl w-full max-w-lg border border-slate-100 relative transition-all duration-500 ${showSuccess ? "opacity-50 scale-95 blur-sm" : ""}`}
      >
        <Link
          to="/"
